...
object ResourceUtil {
    fun copyResourceToDir(resource: String, dir: Path) {
        val uri = ResourceUtil::class.java.getResource(resource)?.toURI()?: throw IllegalArgumentException("Resource not found: $resource")
        ...
        if (Files.exists(dir.resolve(resourcePath.fileName))) {
            Files.delete(dir.resolve(resourcePath.fileName))
        }
        Files.copy(resourcePath, dir.resolve(resourcePath.fileName))
    }
