...
public class IllagerTopHatFeatureRenderer extends FeatureRenderer<IllagerEntity, IllagerEntityModel<IllagerEntity>> {
    ...
    public IllagerTopHatFeatureRenderer(FeatureRendererContext<IllagerEntity, IllagerEntityModel<IllagerEntity>> context, ModelPart modelPart) {
        super(context);
    ...
    public void render(MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, IllagerEntity entity, float limbAngle, float limbDistance, float tickDelta, float animationProgress, float headYaw, float headPitch) {
        ...
        if (!entity.isInvisible() && stack.getItem() == ModItems.TOP_HAT) {
            matrices.push();
